...
2. **直观统计区间**：手动输入 `liuliang` 即可查看当前识别到的网卡、精确的**当月 1 日至当前看账时间**的流量状态。
3. **日志安全防爆**：完美配套 Linux 官方 `logrotate` 机制，自动对后台日志进行每周切割与压缩，防止磁盘空间被写满。
4. **绝对路径与多行过滤**：核心命令全部改用绝对路径并加入 `tail -n 1` 限制，彻底解决后台 `crontab` 环境变量缺失与多条重复防火墙规则导致的报错。

---

## 🛠️ 从零配置步骤

请以 **`root`** 用户身份登录您的 Linux 实例，依次执行以下四个步骤：
...
#### 方案 A：GCP 专属自然月版（当月 1 日至当前时间，推荐 🌟）

* **适用场景**：Google Cloud (GCP) 等严格按照**自然月**（每月 1 号到当月最后一天）计算免费额度和账单的商家。
* **特点**：显示当月 1 号至当前这一秒的精准用量。跨月时脚本首次运行会自动清零计数器。
...
### 第三步：配置快捷查询命令 `liuliang` 与日志防爆策略

1. **设置别名**（从此查流量只需打 6 个字母）：
```bash
...
运行 `crontab -e` 打开计划任务编辑器，移到**最底部**，根据第二步选择的版本，粘贴对应的单行或双行配置：

* **如果选了【方案 A：GCP 专属自然月版】**，粘贴这一行即可：
```text
...
==================================================
 网卡设备: ens4
 统计区间: [ 2026-07-01 00:00:00 ] 至
           [ 2026-07-07 06:36:53 ]
 流量状态: 0.0695 GB / 限额 190 GB
==================================================
...
### 2. 流量超标断网后，如何手动解除封禁恢复网络？

当进入了新的计费周期需要复活网络，或者需要临时扩容解除限制时，直接输入以下两条指令：

```bash
# 1. 清空 TRAFFIC_MONITOR 链中产生的 DROP 封禁规则
